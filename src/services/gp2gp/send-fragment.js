...
export const sendFragment = async (conversationId, odsCode, fragmentMessage, messageId) => {
  ...
  logInfo(
    `Started to send fragment with Message ID: ${messageId}, outbound Conversation ID ${conversationId}.`
  );
  ...
  await axios
    .post(ENDPOINT_URL, requestBody, {headers: {Authorization: gp2gpMessengerAuthKeys}})
    .then(() => {
